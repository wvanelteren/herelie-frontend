import 'dart:async';

import 'package:flutter_bloc/flutter_bloc.dart';

import '../../../domain/repositories/purchase_plan_repository.dart';
import '../../../domain/repositories/recipe_repository.dart';
import 'process_recipe_state.dart';

class ProcessRecipeCubit extends Cubit<ProcessRecipeState> {
  final RecipeRepository recipes;
  final PurchasePlanRepository purchasePlans;

  ProcessRecipeCubit({required this.recipes, required this.purchasePlans})
    : super(const ProcessRecipeState());

  Future<void> process(String text) async {
    if (text.trim().isEmpty) return;
    emit(state.copyWith(status: ProcessStatus.loading, error: null));
    try {
      final recipe = await recipes.processRecipeText(text.trim());
      emit(state.copyWith(status: ProcessStatus.success, recipe: recipe));
      
      // Warm-up purchase-plan generation for processed recipe
      unawaited(
        purchasePlans
            .generateForServings(recipe: recipe, servings: recipe.servings)
            .catchError((_) => null),
      );
    } catch (e) {
      emit(state.copyWith(status: ProcessStatus.failure, error: e.toString()));
    }
  }

  void reset() => emit(const ProcessRecipeState());
}
